...
int main() {
	...
	for (int left = 0; left < m; left++) {
		for (int right = 0; right < n; right++) {
			capacity[leftStart + left][rightStart + right] = arr3[left][right];

			adj[leftStart + left].push_back(rightStart + right);
			adj[rightStart + right].push_back(leftStart + left);

			cost[leftStart + left][rightStart + right] = arr4[left][right];
			cost[rightStart + right][leftStart + left] = -arr4[left][right];
		}
	}
